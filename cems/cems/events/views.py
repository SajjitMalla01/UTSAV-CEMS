...
def home(request):
    ...
    if college:
        events = Event.objects.filter(status="PUBLISHED", college=college).order_by(
            "date_time"
        )
    else:
...
def admin_dashboard(request):
    ...
    User = get_user_model()
    user_qs = (
        User.objects.filter(profile__college=college) if college else User.objects.all()
    )
    ...
    recent_events = event_qs.annotate(reg_count=Count("registrations")).order_by(
        "-created_at"
    )[:8]
    pending_list = (
        event_qs.filter(status="PENDING")
        .order_by("date_time")
    ...
    attendance_rate = round(
        (attended_count / stats["total_regs"] * 100) if stats["total_regs"] > 0 else 0
    )
    ...
    for i in range(5, -1, -1):
        ...
        monthly_regs.append(
            reg_qs.filter(registered_at__gte=m_start, registered_at__lt=m_next).count()
        )
        monthly_events.append(
            event_qs.filter(created_at__gte=m_start, created_at__lt=m_next).count()
        )

    avg_regs = (
        round(stats["total_regs"] / stats["total"], 1) if stats["total"] > 0 else 0
    )

    from accounts.models import EventComment

    recent_comments = (
        EventComment.objects.filter(event__college=college).order_by("-created_at")[:15]
        if college
        else EventComment.objects.all().order_by("-created_at")[:15]
    )
    ...
    all_events = event_qs.annotate(reg_count=Count("registrations")).order_by(
        "-date_time"
    )
    attendance_log = reg_qs.select_related("user", "event").order_by("-registered_at")[
        :50
    ]
    ...
    colleges = (
        College.objects.all() if request.user.is_superuser else College.objects.none()
    )
    if college:
...
def add_comment(request, pk):
    ...
    if text:
        ...
        EventComment.objects.create(
            event=event, user=request.user, text=text, is_pre_event=is_pre
        )
...
def staff_dashboard(request):
    ...
    college = getattr(request, "college", None) or (
        request.user.profile.college if hasattr(request.user, "profile") else None
    )

    if not college and not request.user.is_superuser:
        messages.error(
            request, "No college context found. Please use a college portal link."
        )
        return redirect("home")
    ...
    recent_events = my_events.annotate(reg_count=Count("registrations")).order_by(
        "-created_at"
    )[:8]
    all_my_events = my_events.annotate(reg_count=Count("registrations")).order_by(
        "-created_at"
    )

    # Chart: registrations per event (top 6)
    top_for_chart = list(
        my_events.annotate(rc=Count("registrations")).order_by("-rc")[:6]
    )
    reg_chart_labels = json.dumps(
        [e.title[:18] + (".." if len(e.title) > 18 else "") for e in top_for_chart]
    )
    ...
    for ev in my_events.filter(status="PUBLISHED").order_by("-date_time"):
        regs = (
            Registration.objects.filter(event=ev)
            .select_related("user")
...
def student_dashboard(request):
    ...
    if target_college:
        college_wall = EventComment.objects.filter(
            event__college=target_college
    ...
    context = {
        ...
        "upcoming_events": [
            e for e in registered_events if e.date_time > timezone.now()
        ],
        "upcoming_registrations": [
            r
            for r in regs
            if r.status in ["REGISTERED", "PENDING"]
            and r.event.date_time > timezone.now()
        ],
        "events_data": json.dumps(events_data),
...
def event_list(request):
    ...
    try:
        profile = request.user.profile
        base_qs = (
            Event.objects.filter(college=college) if college else Event.objects.all()
        )
    ...
    return render(
        request, "events/event_list.html", {"events": events, "college": college}
    )
...
def event_create(request):
    ...
    if request.method == "POST":
        ...
        form = EventForm()
    return render(
        request, "events/event_form.html", {"form": form, "title": "Create New Event"}
    )
...
def event_update(request, pk):
    ...
    if not (is_owner or is_global_admin or is_college_admin):
        messages.error(
            request, "Permission denied: You do not have permission to edit this event."
        )
        return redirect("event_detail", pk=pk)
    ...
    return render(
        ...
        {"form": form, "title": f"Edit: {event.title}"},
    )
...
def approve_event(request, pk):
    ...
    if not (is_global_admin or is_college_admin):
        messages.error(
            request,
            "Permission denied: Only admins of this college can approve events.",
        )
...
def publish_event(request, pk):
    ...
    if not (is_global_admin or is_college_admin):
        messages.error(
            request,
            "Permission denied: Only admins of this college can publish events.",
        )
        return redirect("admin_dashboard")
...
def update_event_status(request, pk):
    try:
        ...
        if status not in {"APPROVED", "REJECTED", "PUBLISHED", "PENDING"}:
            return JsonResponse(
                {"success": False, "message": f"Invalid status: {status}"}
            )
