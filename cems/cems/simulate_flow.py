...
def simulate():
    ...
    if resp.url == create_url:
        ...
        errs = BeautifulSoup(resp.text, "html.parser").find_all(
            "ul", class_="errorlist"
        )
        if errs:
            for err in errs:
    ...
    try:
        ...
        print(
            f"   Note: Registration action encountered a constraint or was not a separate form: {e}"
        )
