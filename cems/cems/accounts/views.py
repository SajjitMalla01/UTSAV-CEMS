...
def register(request, college_slug=None):
    ...
    if request.user.is_authenticated:
        try:
            ...
            if req_college and curr_college and req_college.id != curr_college.id:
                messages.warning(
                    request,
                    f"You are currently logged in as a member of {curr_college.name}. Please logout to register with {req_college.name}.",
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            domain_college = College.objects.filter(
            ...
            target_college = (
                form.cleaned_data.get("college") or domain_college or college
            )
...
def login_view(request, college_slug=None):
    ...
    if request.user.is_authenticated:
        try:
            ...
            if req_college and curr_college and req_college.id != curr_college.id:
                messages.warning(
                    request,
                    f"Access Conflict: You are already logged in as a member of {curr_college.name}. You must logout to sign into {req_college.name}.",
                )
    ...
    if college_id:
        ...
        try:
            ...
            messages.info(
                request, f"Logging in for {college.name} administrative portal."
            )
        except Exception:
            pass
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if not profile.is_approved:
                messages.warning(
                    request,
                    "Your staff account is pending administrative approval. Please wait for an administrator to activate your access.",
                )
...
def verify_email(request):
    ...
    if not user_id:
        messages.warning(
            request, "No pending verification found. Please log in or register."
        )
        return redirect("login")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user.profile.role == "STAFF" and not user.profile.is_approved:
                ...
                messages.success(
                    request,
                    "Email verified! Your staff account is now pending administrative approval. You will be able to log in once an admin approves your request.",
                )
                return redirect("login")
...
def approve_staff(request, user_id):
    ...
    if (
        ...
        and staff_profile.college != request.user.profile.college
    ):
        messages.error(
            request, "Permission denied: Cannot approve staff for another college."
        )
        return redirect("admin_dashboard")
...
def reject_staff(request, user_id):
    ...
    if (
        ...
        messages.error(
            request, "Permission denied: Cannot reject staff for another college."
        )
        return redirect("admin_dashboard")
